...
REGVERSION ?= $(call get_gh_version,genuinetools/reg,REGVERSION)
...
BUILDFLAGS ?= \
	...
	--build-arg REGVERSION=$(if $(REGVERSION),$(REGVERSION),$(error REGVERSION is not defined)) \
	--build-arg REGARCH=$(call get_reg_arch) \
...
PORTFLAGS  := -p 5000:5000 -p 5001:5001# --net=host # so other local containers can find it without explicit linking, needs firewall cleared
PUID       := $(shell id -u)
...
OTHERFLAGS := \
	...
	-e PGID=$(PGID) \
	-e PUID=$(PUID) \
	# -e CONF_YML=/data/config.yml \
	# -e REGISTRY_AUTH_HTPASSWD_PATH=/data/auth/.htpasswd \
	# -e REGISTRY_AUTH_HTPASSWD_REALM=Basic-Realm \
	# -e REGISTRY_HTTP_HOST=https://localhost:5000 \
	# -e REGISTRY_HTTP_TLS_CERTIFICATE=/data/auth/certificate.crt \
	# -e REGISTRY_HTTP_TLS_KEY=/data/auth/privatekey.pem \
	# -e REGISTRY_STORAGE_FILESYSTEM_ROOTDIRECTORY=/data/storage \
	# -e REG_ASSET_PATH=/data/web \
	# -e REG_PASS=insecurebydefault \
	# -e REG_USER=registry \
	# -e SSLSUBJECT="/C=US/ST=NY/L=EXAMPLE/O=EXAMPLE/OU=REGISTRY/CN=localhost/emailAddress=everybodycanseethis@mailinator.com"
	# --net=host \
	# -e TZ=Asia/Kolkata \
